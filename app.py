...
def register_student():
    if request.method == 'POST':
        try:
            ...
            db.session.rollback()
            flash('Registration failed. Email or roll nuber already exists.','error')
...
# for company in admin dashboard    
...
def reject_company(id):
 ...
 # for company blacklist in admin
...
def delete_company(id):
    ...
    db.session.commit()
    flash(f'company deleted!','success')
    return redirect(url_for('admin_manage_companies'))
...
def admin_manage_students():
    ...
    if search:
        students=Student.query.filter(
            ...
            (Student.roll_number.contains(search)) |     # this is for "or" for "|"
...
def delete_student(id):
    ...
    db.session.commit()
    flash(f'student deleted!','success')
    return redirect(url_for('admin_manage_students'))
...
# for delete any drive 
@app.route('/admin/drive/delete/<int:id>')
...
# for see all students applications
...
def company_dashboard():
    ...
    for drive in drives:
        ...
        drive_stats.append({'drive':drive, 'applicants':application_count})

    return render_template('company/dashboard.html',company=company,drive_stats=drive_stats)


@app.route('/company/create_drives',methods=['GET','POST'])
def create_drive():    
    if session.get('role')!='company':
        return redirect(url_for('login'))
    ...
    if request.method=='POST':
        try:
            ...
            db.session.commit()
            flash('Drive created! Waiting for admin approval.','sucesss')
            return redirect(url_for('company_dashboard'))
        except Exception as e:
            db.session.rollback()
            flash('Faild to create drive.','error')
...
def edit_drive(id):
    ...
    if request.method=='POST':
        ...
        db.session.commit()
        flash('Drive updated! Wating for admin re-approval.','success')
...
def apply_drive(id):
    ...
    if drive.application_deadline <date.today():
        flash('Application deadline has been passed!','error')
        return redirect(url_for('student_view_drives'))
    ...
    db.session.commit()
    flash('Appliaction submitted successfully!','success')
    return redirect(url_for('student_dashboard'))
...
def student_profile():
    ...
    if request.method == 'POST':
        ...
        student.cgpa = request.form.get('cgpa', 0)
